...
// Choosing to have no mounting with hotend_mount=0, which suits Buddaschnozzles and the 
// makerspace hot-end with the adaptor plate.
...
module block_holes(legacy_mount=false){
	...
	translate(motor_mount_translation){
		translate([-gear_separation,0,0]){
			...
			rotate([90,0,0])
			rotate(360/16)
			cylinder(r=filament_feed_hole_d/2,h=wade_block_depth*3,center=true,$fn=30);	
